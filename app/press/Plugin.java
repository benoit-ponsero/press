...
public class Plugin extends PlayPlugin {
    ...
    @Override
    public void onLoad() {
        lessCompiler = new LessCompiler(Play.mode == Play.Mode.DEV);
    ...
    private void handleResponse(VirtualFile file, Request request, Response response) {
        long lastModified = lessCompiler.lastModifiedRecursive(file.getRealFile());
    ...
    private void handleOk(Request request, Response response, VirtualFile file, String etag, long lastModified) {
        response.status = 200;
        response.print(lessCompiler.get(file.getRealFile()));
